...
<div class="header">My Anime List</div>
	...
	<c:if test="${status =='ongoing'}">
		<h2 >All Ongoing Anime</h2>
	</c:if>
	<!--------->
	<c:if test="${status =='completed'}">
		<h2>All Completed Anime</h2>
	</c:if>	
	<!--------->
	<c:if test="${status =='towatch'}">
		<h2>All To Watch Anime</h2>
	</c:if>
	              	 ...
	              	 <div class="row">
	              	 	<div class="column">
	              	 		...
	              	 		<div align="center" style="box-shadow: 0 0 20px #000; margin-bottom: 15px;">
	              	 			<img  onerror = "imagenotfound1(this)" src="/docs/images/anime/<c:out value="${anime.image}" />" id="im"  >
	              	 		</div>
	              	 	</div>
...
window.onload = function() {
	...
	// Inside 2nd organize method in case of re-styling
	
	let saved_success = ${saved};
	const first_status = "&#9776;";
	const second_status = '&#9783;';
	...
	if (first_status != '${coding}') {
		document.body.style.backgroundImage = "repeating-linear-gradient(132deg, black , #4c4c4c 14%, black 20%)";
	...
	if (!${saved}){
		alert("Anime did not save because it already exists");
	}	
}
</script>
<script>
